    ...
    <header className="w-full h-[60px] px-10 py-4 flex items-center border-b border-gray-200">
      ...
      <div className="w-1/2 flex gap-6 items-center justify-end">
        <nav className="flex gap-6">
          ...
          {menu.map((link) => (
            <Link key={link.id} href={link.href} className="text-sm">
              {link.name}
            </Link>
          ))}
        ...
        <div>
          <button className="bg-green-800 text-white px-4 py-2 rounded-full cursor-pointer text-sm hover:bg-green-600">
            Get Started
          </button>
        </div>
      </div>
